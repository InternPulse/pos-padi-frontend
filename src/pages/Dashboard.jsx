...
function Dashboard() {
  ...
  const [rawAgentsData, setRawAgentsData] = useState(() => {
    return transformAgents([]).rawAgents;
  });
  ...
  const processedAgents = rawAgents.map((agent) => {
    const agentRevenue = transactions
      .filter((tx) => tx.agent == `${agent.firstName} ${agent.lastName}`)
      .reduce((acc, tx) => acc + tx.fee, 0);
    return { ...agent, revenue: agentRevenue };
  });

  const topAgent = processedAgents.sort(function (a, b) {
  ...
  function SelectDateContainer({ onDateRangeChange }) {
    return <DateRangePicker onDateRangeChange={onDateRangeChange} />;
  }
  ...
  function TopAgentContainer() {
    const topAgentDetails = {
      ...
      clients: rawCustomers.filter(
        (customer) =>
          customer.agent == `${topAgent.firstName} ${topAgent.lastName}`
      ).length,
    };
  ...
  const CompanySummaries = () => {
    const totalAmount = periodFilteredTransactions.reduce(
    ...
    return [
      ...
      {
        title: "Customers",
        amount: [...new Set(periodFilteredTransactions.map((t) => t.customer))]
          .length,
        icon: <GrGroup />,
  ...
  const agentsSummary = {
    ...
    percent: percentageDiff(rawAgents, "customers", "length", activePeriod)
      .percentageChange,
    period: percentageDiff(rawAgents, "customers", "length", activePeriod)
      .interval,
  };
  ...
  return (
    ...
    >
      ...
      <Flex
        ...
        gap={{ base: 3, md: 5 }}
        justify={
          user.role == "admin" ? "center" : { base: "center", md: "start" }
        }
      >
